...
#  Description: Recursion practice problems
...
def groupSumClump(start, nums, target):
  if (start >= len(nums)):
    ...
    return groupSumClump(start + count, nums, target - totSum) or groupSumClump(start + count, nums, target)
...
def splitArray(nums):
  ...
  splitArrayHelper(nums, listA, listB, 0)

  for item in listB:
    if (sum(nums) - item == item):
      cond = True

  return cond


def splitArrayHelper(i, j, k, l):
  if (l == len(i)):
    k.append(sum(j))
    ...
    splitArrayHelper(i, j, k, l + 1)
    splitArrayHelper(i, m, k, l + 1)
...
def splitOdd10(nums):
  ...
  return splitOdd10Helper(nums, emptyList, 0)


def splitOdd10Helper(i, j, index):
  if index == len(i):
    if ((sum(j) % 10 == 0) and ((sum(i) - sum(j)) % 2 != 0)):
      return True
    return False
    ...
    return (splitOdd10Helper(i, j, index + 1) or splitOdd10Helper(i, k, index + 1))
...
def split53(nums):
  threeSum, fiveSum = 0, 0

  return split53Helper(nums, 0, threeSum, fiveSum)


def split53Helper(i, j, k, l):
  if j == len(i):
    if k == l:
      return True
    return False
    ...
    if i[j] % 3 == 0:
      return split53Helper(i, j + 1, k + i[j], l)

    if i[j] % 5 == 0:
      return split53Helper(i, j + 1, k, l + i[j])

    else:
      return ((split53Helper(i, j + 1, k + i[j], l) or (split53Helper(i, j + 1, k, l + i[j]))))
